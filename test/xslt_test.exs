defmodule XsltTest do
  ...
  test "Happy path using param --xml-as-string" do
    xml = File.read!(Path.expand("./test/fixtures/product/simple_example.xml"))
    template = Path.expand("./test/fixtures/product/xslt_template.xml")
    assert {:ok, _xml} = Xslt.transform(template, xml, "--xml-as-string")
  end
  ...
  test "Guards should return a bad_arguments error" do
    assert {:error, :bad_arguments} =
             Xslt.transform(:anything, :anything, "--stringparam param value")
  end
end

defmodule Xslt do
  ...
  your XML files. It can also be called with the pathname to the template file and
  the xml as string. It will then return the transformed XML in response.
  ...
  ## Examples:
      ...
      iex> xml = File.read!(Path.expand("./test/fixtures/product/simple_example.xml"))
      ...> template = Path.expand("./test/fixtures/product/xslt_template.xml")
      ...> {:ok, html} = Xslt.transform(template, xml, "--xml-as-string")
      ...> Regex.replace(~r/\\n/, html, "")
      "<html> <head><title>Book Review</title></head> <body>  <bookreview>   <title lorem=\\"ipsum\\">wewt</title>  </bookreview> </body></html>"
  ...
  def transform(template, xml, "--xml-as-string")
      when is_bitstring(template) and is_bitstring(xml) do
    Porcelain.shell("echo '#{xml}' | xsltproc '#{template}' -")
    |> handle_output
  end

  def transform(template, xml, params)
      when is_bitstring(template) and is_bitstring(xml) and is_bitstring(params) do
    Porcelain.shell("xsltproc #{params} '#{template}' '#{xml}'")
    |> handle_output
  end

  def transform(template, xml, _) when is_bitstring(template) and is_bitstring(xml),
    do: transform(template, xml)
